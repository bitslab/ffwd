...
DIR="$RESULTS_DIR/lazylist_size_bench"
LOG=$DIR/lazylist_size_log.txt
BIN="../bin"

NPROC=`nproc`
NUM_OF_THREADS=$NPROC

function list_size_exp {
  ...
  size=$START;
    ...
    do
        ...
        echo -n "scale=6;(" > sum
        echo -n > list
	    echo "sudo $EXECUTABLE -t $NUM_OF_THREADS -d $DURATION -i $size -r $range -u $UPDATE_RATIO" >> $LOG
        for j in `seq 1 $NUMBER_OF_RUNS`
        do
            sudo $EXECUTABLE -t $NUM_OF_THREADS -d $DURATION -i $size -r $range -u $UPDATE_RATIO| tr -d ',\n' | tee -a sum list >> /dev/null
...
if [ $# -eq 0 ]; then
    ...
    header=`echo $t | tr '[:lower:]' '[:upper:]'`
		...
		list_size_exp $t | tee -a $EXPERIMENT_NAME.csv
	done
else
	for t in "$@"; do
	  EXPERIMENT_NAME="$DIR/$t"
