...
#ifdef SSYNC
				if (set_add(d->set, val, TRANSACTIONAL, &client_data1, &client_data2)) {
#elif FFWD
...
#ifdef SSYNC
					if (set_remove(d->set, last, TRANSACTIONAL, &client_data1, &client_data2)) {
#elif FFWD
...
#ifdef SSYNC
					if (set_remove(d->set, val, TRANSACTIONAL, &client_data1, &client_data2)) {
#elif FFWD
...
#ifdef SSYNC
			if (set_contains(d->set, val, TRANSACTIONAL, &client_data1, &client_data2)) 
#elif FFWD
...
int main(int argc, char **argv) {
	...
	//slab_init(&s, sizeof(struct node));
#ifdef FFWD
	ffwd_init();
  launch_servers(1);
#endif
...
#ifdef SSYNC
		if (set_add_2(set, val)) {
#elif FFWD
		if (set_seq_add_2(set, val)) {
#endif
			last = val;
			i++;
		}
