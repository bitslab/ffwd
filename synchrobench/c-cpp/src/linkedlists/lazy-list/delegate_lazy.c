...
extern liblock_lock_t g_liblock;
#endif
...
int parse_insert_intern(struct context1 *ctx1)  
{
  node_l_t *pred = ctx1->pred;
  node_l_t *curr = ctx1->curr;
  val_t val = ctx1->val;
	node_l_t *newnode;
	int result = 1;
...
int parse_insert(intset_l_t *set, val_t val) 
{
	...
	//FFWD_EXEC_3(&parse_insert_intern, 3, pred, curr, val, return_value);
  struct context1 ctx1;
  ctx1.pred = pred;
  ctx1.curr = curr;
  ctx1.val = val;
  int return_value = liblock_exec(&g_liblock, (void * (*)(void *))&parse_insert_intern, (void *)(uintptr_t)(&ctx1));
	return return_value;
}
 ...
 * free the memory.
 */
int parse_delete_intern(struct context2 *ctx2)
{
	int result = 1;
  node_l_t *pred = ctx2->pred;
  node_l_t *curr = ctx2->curr;
  val_t val = ctx2->val;
...
int parse_delete(intset_l_t *set, val_t val) 
{
	...
	//GET_CONTEXT();
	//int return_value; //this is going to hold the function return value from ffwd_exec
	//FFWD_EXEC_3(&parse_delete_intern, 3, pred, curr, val, return_value);
  struct context2 ctx2;
  ctx2.pred = pred;
  ctx2.curr = curr;
  ctx2.val = val;
  int return_value = liblock_exec(&g_liblock, (void * (*)(void *))&parse_delete_intern, (void *)(uintptr_t)(&ctx2));
	return return_value;
}
