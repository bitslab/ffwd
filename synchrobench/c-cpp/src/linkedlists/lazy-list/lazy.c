...
#ifdef SSYNC
int parse_insert(intset_l_t *set, val_t val, lock_local_data *data1, lock_local_data *data2) 
#else
...
#endif
{
	...
	LOCK(&pred->lock, data1);
	LOCK(&curr->lock, data2);
	...
	UNLOCK(&curr->lock, data2);
	UNLOCK(&pred->lock, data1);
 ...
 */
#ifdef SSYNC
int parse_delete(intset_l_t *set, val_t val, lock_local_data *data1, lock_local_data *data2) 
#else
