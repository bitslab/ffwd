...
int ht_contains(ht_intset_t *set, int val, int transactional, lock_local_data *data1, lock_local_data *data2) {
	int addr;
	
	/* Get key */
	addr = val % maxhtlength;
	return set_contains(set->buckets[addr], val, transactional, data1, data2);
}

int ht_add(ht_intset_t *set, int val, int transactional, lock_local_data *data1, lock_local_data *data2) {
	int addr, result;
	
	/* Get key */
	addr = val % maxhtlength;
	result = set_add(set->buckets[addr], val, transactional, data1, data2);
	return result;
}

int ht_remove(ht_intset_t *set, int val, int transactional, lock_local_data *data1, lock_local_data *data2) {
	int addr, result;
	
	/* Get key */
	addr = val % maxhtlength;
	result = set_remove(set->buckets[addr], val, transactional, data1, data2);
	
	return result;
}

int ht_add_2(ht_intset_t *set, int val, int transactional) {
	int addr, result;
	
	/* Get key */
	addr = val % maxhtlength;
	result = set_add_2(set->buckets[addr], val);
	return result;
}
#if 0
/* 
...
int ht_move(ht_intset_t *set, int val1, int val2, int transactional) {
	node_t *pred1, *curr1, *curr2, *pred2, *newnode;
