...
{
	int addr, server;
	int quarter = maxhtlength/4;
	
	/* Get key */
	addr = val % maxhtlength;
	if (addr < quarter)
		server = 0;
	else if (addr < 2*quarter)
		server = 1;
	else if (addr < 3*quarter)
		server = 2;
	else
		server = 3;
	return set_contains(set->buckets[addr], val, server);
}

int ht_add(ht_intset_t *set, int val, int transactional) 
{
	int addr, result, server;
	int quarter = maxhtlength/4;
	...
	if (addr < quarter)
		...
		server = 3;
	result = set_add(set->buckets[addr], val, server);
	return result;
}
...
int ht_add_2(ht_intset_t *set, int val, int transactional) 
{
	...
	/* Get key */
	addr = val % maxhtlength;
	result = set_seq_add_2(set->buckets[addr], val);
	return result;
}
...
int ht_remove(ht_intset_t *set, int val, int transactional) {
	...
	if (addr < quarter)
		...
		server = 3;
	result = set_remove(set->buckets[addr], val, server);
