...
void *list_global_init(int init_size, int value_range)
{
	...
	int i, key, val, direction, j;

	for (j = 0; j< MAX_SERVERS; j++){
		tree[j] = (rlu_tree_t *)numa_alloc_onnode(sizeof(rlu_tree_t), j);

		if (tree[j] == NULL){
			return NULL;
		}
		tree[j]->root = rlu_new_node(INT_MAX);

		i = 0;
		while (i < (init_size/MAX_SERVERS)) {
...
uint64_t list_ins_ffwd(int key)
{
	...
	prev = (node_t *)(tree[key%MAX_SERVERS]->root);
...
int list_ins(int key, pthread_data_t *data)
{
	...
	struct ffwd_context *context = (struct ffwd_context *)data;
	FFWD_EXEC(key%MAX_SERVERS, list_ins_ffwd, ret, 1, key)
...
int list_del(int key, pthread_data_t *data)
{
	...
	struct ffwd_context *context = (struct ffwd_context *) data;
	FFWD_EXEC(key%MAX_SERVERS, list_del_ffwd, ret, 1, key)
...
uint64_t list_find_ffwd(int key)
{
	...
	cur = (node_t *)(tree[key%MAX_SERVERS]->root);
	cur = (node_t *)(cur->child[0]);
...
int list_find(int key, pthread_data_t *data)
{
	...
	struct ffwd_context *context = (struct ffwd_context *) data;
	FFWD_EXEC(key%MAX_SERVERS, list_find_ffwd, ret, 1, key)
