...
void func42(struct context42 *ctx42);
void func42(struct context42 *ctx42) {
    PREFIX_STATS *pfs;
    pfs = stats_prefix_find(ctx42->key, ctx42->nkey);
    if (NULL != pfs) {
        pfs->num_gets++;
        if (ctx42->is_hit) {
            pfs->num_hits++;
...
void stats_prefix_record_get(const char *key, const size_t nkey, const bool is_hit) {
    ...
    struct context42 ctx42;
    ctx42.key = key;
    ctx42.nkey = nkey;
    ctx42.is_hit = is_hit;
    FFWD_EXEC(0, &func42, return_value, 1, &ctx42)
    //STATS_LOCK();
    //STATS_UNLOCK();
}
...
void func43(struct context43 *ctx43);
void func43(struct context43 *ctx43) {
    PREFIX_STATS *pfs;
    pfs = stats_prefix_find(ctx43->key, ctx43->nkey);
    if (NULL != pfs) {
        pfs->num_deletes++;
    }
}
...
void stats_prefix_record_delete(const char *key, const size_t nkey) {
    ...
    GET_CONTEXT()
    uint64_t return_value;
    struct context43 ctx43;
    ctx43.key = key;
    ctx43.nkey = nkey;
    FFWD_EXEC(0, &func43, return_value, 1, &ctx43)
    //STATS_LOCK();
    //STATS_UNLOCK();
}
...
void func44(struct context44 *ctx44);
void func44(struct context44 *ctx44) {
    PREFIX_STATS *pfs;
    pfs = stats_prefix_find(ctx44->key, ctx44->nkey);
    if (NULL != pfs) {
        pfs->num_sets++;
    }
}
...
void stats_prefix_record_set(const char *key, const size_t nkey) {
    ...
    GET_CONTEXT()
    uint64_t return_value;
    struct context44 ctx44;
    ctx44.key = key;
    ctx44.nkey = nkey;
    FFWD_EXEC(0, &func44, return_value, 1, &ctx44)
    //STATS_LOCK();
    //STATS_UNLOCK();
}
...
void func45(struct context45 *ctx45);
void func45(struct context45 *ctx45) {
    ctx45->ret = 0;
    ...
    for (i = 0; i < PREFIX_HASH_SIZE; i++) {
        for (pfs = prefix_stats[i]; NULL != pfs; pfs = pfs->next) {
            written = snprintf(ctx45->buf + ctx45->pos, size-ctx45->pos, format,
...
char *stats_prefix_dump(int *length) {
    ...
    GET_CONTEXT()
    uint64_t return_value;
    struct context45 ctx45;
    FFWD_EXEC(0, &func45, return_value, 1, &ctx45)
    if (ctx45.ret == 1) return NULL;
    pos = ctx45.pos;
    buf = ctx45.buf;
    //STATS_LOCK();
    //STATS_UNLOCK();
