...
/*@null@*/
/* Has both delegated & non-delegated versions */
item *do_item_alloc_del(char *key, const size_t nkey, const int flags, const rel_time_t exptime, const int nbytes) {
    ...
    for (search = tails[id];
        ...
        if (search->refcount == 0 &&
            ...
            it->refcount = 1;
            slabs_adjust_mem_requested_del(it->slabs_clsid, ITEM_ntotal(it), ntotal);
            do_item_unlink_del(it);
    ...
    if (it == NULL && (it = slabs_alloc_del(ntotal, id)) == NULL) {
        /*
        ...
        */
        ...
        for (search = tails[id]; tries > 0 && search != NULL; tries--, search=search->prev) {
            if (search->refcount == 0) {
                ...
                do_item_unlink_del(search);
                break;
            }
        }
        it = slabs_alloc_del(ntotal, id);
        if (it == 0) {
            itemstats[id].outofmemory++;
...
void item_free_del(item *it) {
    ...
    DEBUG_REFCNT(it, 'F');
    slabs_free_del(it, ntotal, clsid);
}
...
void do_item_unlink_del(item *it) {
    ...
    if ((it->it_flags & ITEM_LINKED) != 0) {
        ...
        item_unlink_q(it);
        if (it->refcount == 0) item_free_del(it);
    }
}
...
int do_item_replace_del(item *it, item *new_it) {
    ...
    do_item_unlink_del(it);
    return do_item_link_del(new_it);
}
...
item *do_item_get_del(const char *key, const size_t nkey) {
    ...
    if (it != NULL && settings.oldest_live != 0 && settings.oldest_live <= current_time &&
        ...
        do_item_unlink_del(it);           /* MTSAFE - cache_lock held */
        it = NULL;
    }
