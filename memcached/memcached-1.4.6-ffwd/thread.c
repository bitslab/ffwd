...
#ifdef FFWD
    ffwd_thread_create(&thread, &attr, func, arg);
#else
...
static void *worker_libevent(void *arg) {
    GET_CONTEXT()
    void *arg_new = context->initvalue;
    LIBEVENT_THREAD *me = arg_new;
...
void func1(struct context1 *ctx1);
void func1(struct context1 *ctx1) {
  ctx1->it = do_item_alloc_del(ctx1->key, ctx1->nkey, ctx1->flags, ctx1->exptime, ctx1->nbytes);
}
...
item *item_alloc(char *key, size_t nkey, int flags, rel_time_t exptime, int nbytes) {
    ...
    ctx1.nkey = nkey;
    ctx1.flags = flags;
    ctx1.exptime = exptime;
    ctx1.nbytes = nbytes;
    //it = do_item_alloc_del(key, nkey, flags, exptime, nbytes);
    FFWD_EXEC(0, &func1, return_value, 1, &ctx1)
    //key = ctx1.key;
    //nkey = ctx1.nkey;
    //flags = ctx1.flags;
    //exptime = ctx1.exptime;
    //nbytes = ctx1.nbytes;
    it = ctx1.it;
    return it;
}
...
void func3(struct context3 *ctx3);
void func3(struct context3 *ctx3) {
  ctx3->it = do_item_get_del(ctx3->key, ctx3->nkey);
}
/*
...
item *item_get(const char *key, const size_t nkey) {
    ...
    FFWD_EXEC(0, &func3, return_value, 1, &ctx3)
    //key = ctx3.key;
    it = ctx3.it;
    return it;
}
...
void func5(struct context5 *ctx5);
void func5(struct context5 *ctx5) {
  ctx5->ret = do_item_link_del(ctx5->item);
}
...
int item_link(item *item) {
    ...
    GET_CONTEXT()
    uint64_t return_value;
    struct context5 ctx5;
    ctx5.item = item;
    //ret = do_item_link(item);
    FFWD_EXEC(0, &func5, return_value, 1, &ctx5)
    ret = ctx5.ret;
    return ret;
}
...
void item_remove(item *item) {
    ...
    GET_CONTEXT()
    uint64_t return_value;
    FFWD_EXEC(0, &do_item_remove_del, return_value, 1, item)
#else
    do_item_remove_del(item);
#endif
}
 ...
 */
/* Always called inside delegated functions */
int item_replace_del(item *old_it, item *new_it) {
    return do_item_replace_del(old_it, new_it);
}
...
void item_unlink(item *item) {
    GET_CONTEXT()
    uint64_t return_value;
    FFWD_EXEC(0, &do_item_unlink_del, return_value, 1, item)
    //do_item_unlink(item);
}
...
void item_update(item *item) {
    GET_CONTEXT()
    uint64_t return_value;
    FFWD_EXEC(0, &do_item_update, return_value, 1, item)
    //do_item_update(item);
}
...
void func8(struct context8 *ctx8);
void func8(struct context8 *ctx8) {
  ctx8->ret = do_add_delta_del(ctx8->c, ctx8->key, ctx8->nkey, ctx8->incr, ctx8->delta, ctx8->buf, ctx8->cas);
}
...
enum delta_result_type add_delta(conn *c, const char *key,
    ...
    ctx8.buf = buf;
    ctx8.cas = cas;
    FFWD_EXEC(0, &func8, return_value, 1, &ctx8)
...
void func9(struct context9 *ctx9) {
  ctx9->ret = do_store_item_del(ctx9->item, ctx9->comm, ctx9->c);
}
...
void item_flush_expired() {
    GET_CONTEXT()
    uint64_t return_value;
    FFWD_EXEC(0, &do_item_flush_expired_del, return_value, 0)
...
void func10(struct context10 *ctx10) {
  ctx10->ret = do_item_cachedump(ctx10->slabs_clsid, ctx10->limit, ctx10->bytes);
}
/*
 ...
 */
void  item_stats_del(ADD_STAT add_stats, void *c) {
    do_item_stats_del(add_stats, c);
 ...
 */
void  item_stats_sizes_del(ADD_STAT add_stats, void *c) {
    do_item_stats_sizes_del(add_stats, c);
    //do_item_stats_sizes(add_stats, c);
}

/******************************* GLOBAL STATS ******************************/
/* Delegate everything that uses this macro */
...
void threadlocal_stats_reset_del(void) {
    int ii, sid;
...
void threadlocal_stats_aggregate_del(struct thread_stats *stats) {
    int ii, sid;
