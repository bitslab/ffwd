...
void process_tasks(long process_id)
{
 ...
 retry_entry:
    ...
    FFWD_EXEC(0, &func3, return_value, 1, n_processors)
#else
            ...
            if( t )
                {
                    ...
                    GET_CONTEXT()
                    uint64_t return_value = 0;
                    FFWD_EXEC(0, &func4, return_value, 0)
#else
                    {pthread_mutex_lock(&(global->pbar_lock));};
                    global->pbar_count-- ;
                    {pthread_mutex_unlock(&(global->pbar_lock));};
    ...
    if( ctx5->tq->tail == 0 )
        {
            ...
            ctx5->tq->tail    = ctx5->task ;
            ctx5->tq->top     = ctx5->task ;
            ctx5->tq->n_tasks = 1 ;
        }
    else
        {
            /* Usual case */
            if( ctx5->mode == TASK_APPEND )
                {
                    ctx5->tq->tail->next = ctx5->task ;
                    ctx5->tq->tail = ctx5->task ;
                    ctx5->tq->n_tasks++ ;
                }
            else
                {
                    ctx5->task->next = ctx5->tq->top ;
                    ctx5->tq->top = ctx5->task ;
                    ctx5->tq->n_tasks++ ;
                }
        }
...
void func6(struct context6 *ctx6);
void func6(struct context6 *ctx6)
{
  Task *prev ;
  if( ctx6->tq->top )
      {
          if( ctx6->qid == taskqueue_id[ctx6->process_id] )
              {
                  ctx6->t = ctx6->tq->top ;
                  ctx6->tq->top = ctx6->t->next ;
                  if( ctx6->tq->top == 0 )
                      ctx6->tq->tail = 0 ;
                  ctx6->tq->n_tasks-- ;
              }
              ...
              {
                  /* Get tail */
                  for( prev = 0, ctx6->t = ctx6->tq->top ; ctx6->t->next ;
                      prev = ctx6->t, ctx6->t = ctx6->t->next ) ;
            ...
            if( tq->n_tasks > 0 )
                {
                    ...
                    struct context6 ctx6;
                    ctx6.tq = tq;
                    ctx6.t = t;
                    ctx6.qid = qid;
                    ctx6.process_id = process_id;
                    FFWD_EXEC(0, &func6, return_value, 1, &ctx6)
                    tq = ctx6.tq;
                    t = ctx6.t;
#else
                    {pthread_mutex_lock(&(tq->q_lock));};
...
void func7(struct context7 *ctx7);
void func7(struct context7 *ctx7)
{
  ctx7->toBreak = 0;
  long i ;
  if( ctx7->tq->free )
      {
          /* Scan the free list */
          for( i = 1, ctx7->p = ctx7->tq->free ;
              (i < N_ALLOCATE_LOCAL_TASK) && ctx7->p->next ;
              i++, ctx7->p = ctx7->p->next ) ;

          task_struct[ctx7->process_id].local_free_task = ctx7->tq->free ;
          task_struct[ctx7->process_id].n_local_free_task = i ;
          ctx7->tq->free = ctx7->p->next ;
          ctx7->tq->n_free -= i ;
          ctx7->p->next = 0 ;
          ctx7->toBreak = 1;
      }
                    ...
                    if( tq->n_free > 0 )
                        {
                            ...
                            struct context7 ctx7;
                            ctx7.tq = tq;
                            ctx7.process_id = process_id;
                            ctx7.toBreak = 0;
                            FFWD_EXEC(0, &func7, return_value, 1, &ctx7)
                            p = ctx7.p;
                            tq = ctx7.tq;
...
void func8(struct context8 *ctx8)
{
  ctx8->p->next = ctx8->tq->free ;
  ctx8->tq->free = ctx8->top ;
  ctx8->tq->n_free += ctx8->i ;
}
...
/***************************************************************************
 *
 *    check_task_counter_wo_locks()
 *
 *    Check task counter and return TRUE if this is the first task.
 *
 *    Does not have the operations synchronised because the calling function 
 *    itself will ensure the synchronisation
 ****************************************************************************/

long check_task_counter_wo_locks()
{
    long flag = 0 ;
