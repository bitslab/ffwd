...
void func(struct context *ctx)
{
            ...
            struct context ctx;
            FFWD_EXEC(0, &func, return_value, 1, &ctx)
#else
#endif
...
void func15(struct context15 *ctx15)
{
  ctx15->pc1->cost_estimate += ctx15->cost1;
  ctx15->pc1->n_total_inter++;
  ctx15->pc2->cost_estimate += ctx15->cost2;
  ctx15->pc2->n_total_inter++;
  global->cost_estimate_sum += (ctx15->cost1 + ctx15->cost2);
  global->cost_sum += (ctx15->cost1 + ctx15->cost2);
}
...
void func17(struct context17 *ctx17);
void func17(struct context17 *ctx17)
{
  ctx17->prev->next = ctx17->elem->interactions ;
  ctx17->elem->interactions = ctx17->i_list ;
  ctx17->elem->n_interactions += ctx17->i_len ;
}
...
void func18(struct context18 *ctx18);
void func18(struct context18 *ctx18)
{
  ctx18->pc->n_total_inter += ctx18->delta_n_inter ;
}
...
//#ifdef FFWD
            ...
            GET_CONTEXT()
            uint64_t return_value = 0;
            struct context17 ctx17;
            ctx17.elem = elem;
            ctx17.prev = prev;
            ctx17.i_list = i_list;
            ctx17.i_len = i_len;
            FFWD_EXEC(0, &func17, return_value, 1, &ctx17)
            elem = ctx17.elem;
            prev = ctx17.prev;
            i_list = ctx17.i_list;
            i_len = ctx17.i_len;
    ...
    if( delta_n_inter != 0 )
        {
            ...
            GET_CONTEXT()
            uint64_t return_value = 0;
            struct context18 ctx18;
            ctx18.pc = pc;
            ctx18.delta_n_inter = delta_n_inter;
            FFWD_EXEC(0, &func, return_value, 1, &ctx)
            pc = ctx18.pc;
            delta_n_inter = ctx18.delta_n_inter;
#else
...
void func19(struct context19 *ctx19);
void func19(struct context19 *ctx19)
{
  ctx19->i_list = ctx19->e->interactions ;
  ctx19->e->interactions = (Interaction *)0 ;
  ctx19->e->n_interactions = 0 ;
}
...
//#ifdef FFWD
    ...
    GET_CONTEXT()
    uint64_t return_value = 0;
    struct context19 ctx19;
    ctx19.e = e;
    ctx19.i_list = i_list;
    FFWD_EXEC(0, &func19, return_value, 1, &ctx19)
    e = ctx19.e;
    i_list = ctx19.i_list;
#else
    {pthread_mutex_lock(&(e->elem_lock->lock));};
...
void func20(struct context20 *ctx20);
void func20(struct context20 *ctx20)
{
  ctx20->i_list = ctx20->e->vis_undef_inter ;
  ctx20->e->vis_undef_inter = (Interaction *)0 ;
  ctx20->e->n_vis_undef_inter = 0 ;
}
...
//#ifdef FFWD
    ...
    GET_CONTEXT()
    uint64_t return_value = 0;
    struct context20 ctx20;
    ctx20.e = e;
    ctx20.i_list = i_list;
    FFWD_EXEC(0, &func20, return_value, 1, &ctx20)
    e = ctx20.e;
    i_list = ctx20.i_list;
#else
    {pthread_mutex_lock(&(e->elem_lock->lock));};
...
void func16(struct context16 *ctx16);
void func16(struct context16 *ctx16)
{
  global->cost_sum          += ctx16->pc->cost_history[0];
  global->cost_estimate_sum += ctx16->pc->cost_estimate;
}
...
void func21(struct context21 *ctx21);
void func21(struct context21 *ctx21)
{
  ctx21->e->rad_subtree.r += ctx21->ec->rad_subtree.r * (float)0.25 ;
  ctx21->e->rad_subtree.g += ctx21->ec->rad_subtree.g * (float)0.25 ;
  ctx21->e->rad_subtree.b += ctx21->ec->rad_subtree.b * (float)0.25 ;
  ctx21->e->join_counter-- ;
  ctx21->join_flag = (ctx21->e->join_counter == 0) ;
}
...
void func28(struct context28 *ctx28)
{
  global->total_energy.r += ctx28->ec->rad.r * ctx28->ec->area ;
  global->total_energy.g += ctx28->ec->rad.g * ctx28->ec->area ;
  global->total_energy.b += ctx28->ec->rad.b * ctx28->ec->area ;
  global->total_patch_area += ctx28->ec->area ;
}
...
//#ifdef FFWD
            ...
            GET_CONTEXT()
            uint64_t return_value = 0;
            struct context21 ctx21;
            ctx21.e = e;
            ctx21.ec = ec;
            ctx21.join_flag = join_flag;
            FFWD_EXEC(0, &func21, return_value, 1, &ctx21)
            e = ctx21.e;
            ec = ctx21.ec;
            join_flag = ctx21.join_flag;
#else
            {pthread_mutex_lock(&(e->elem_lock->lock));};
    ...
    GET_CONTEXT()
    uint64_t return_value = 0;
    struct context16 ctx16;
    ctx16.pc = pc;
    FFWD_EXEC(0, &func16, return_value, 1, &ctx16)
    pc = ctx16.pc;
#else
    {pthread_mutex_lock(&(global->cost_sum_lock));};
...
void func29(struct context29 *ctx29)
{
    ...
    ctx29->p = global->free_element ;
    global->free_element = ctx29->p->center ;
...
void func22(struct context22 *ctx22);
void func22(struct context22 *ctx22)
{
  ctx22->leaf  = _LEAF_ELEMENT(ctx22->elem) ;
}
...
//#ifdef FFWD
    ...
    GET_CONTEXT()
    uint64_t return_value = 0;
    struct context22 ctx22;
    ctx22.leaf = leaf;
    ctx22.elem = elem;
    FFWD_EXEC(0, &func22, return_value, 1, &ctx22)
    leaf = ctx22.leaf; 
#else
    {pthread_mutex_lock(&(elem->elem_lock->lock));};
...
void func23(struct context23 *ctx23)
{
  ctx23->inter->next = ctx23->elem->interactions ;
  ctx23->elem->interactions = ctx23->inter ;
  ctx23->elem->n_interactions++ ;
}
...
{
    if( ctx24->prev == 0 )
        ctx24->elem->interactions = ctx24->inter->next ;
    else
        ctx24->prev->next = ctx24->inter->next ;
    ctx24->elem->n_interactions-- ;
}
...
void func25(struct context25 *ctx25)
{
  ctx25->inter->next = ctx25->elem->vis_undef_inter ;
  ctx25->elem->vis_undef_inter = ctx25->inter ;
  ctx25->elem->n_vis_undef_inter++ ;
}
...
{
  if( ctx26->prev == 0 )
      ctx26->elem->vis_undef_inter = ctx26->inter->next ;
  else
      ctx26->prev->next = ctx26->inter->next ;
  ctx26->elem->n_vis_undef_inter-- ;
}
...
void func30(struct context30 *ctx30)
{
  ...
  /* Get an element data structure */
  ctx30->p = global->free_interaction ;
  global->free_interaction = ctx30->p->next ;
...
void func31(struct context31 *ctx31)
{
  ...
  ctx31->interaction->next = global->free_interaction ;
  global->free_interaction = ctx31->interaction ;
  global->n_free_interactions++ ;
}
