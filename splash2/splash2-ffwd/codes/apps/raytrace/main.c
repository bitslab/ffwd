...
VOID	StartRayTrace()
  ...
  FFWD_EXEC(0, &func1, return_value, 1, &pid)
#else
...
/* ReadEnvFile calls GlobalMalloc() which has FFWD_EXEC calls. So, 
 * ffwd_init() & ffwd_bind_main_thread() needs to be called earlier 
 */
#ifdef FFWD
  ffwd_init();
  launch_servers(1);
...
#ifdef FFWD
		ffwd_thread_create(&PThreadTable[i], NULL, (void * (*)(void *))(StartRayTrace), 0);
#else
    long Error;
