...
//void func3(WPJOB **ctx3_wpentry, INT ctx3_pid, int *ctx3_to_ret, int *ctx3_retval);
//void func3(WPJOB **ctx3_wpentry, INT ctx3_pid, int *ctx3_to_ret, int *ctx3_retval)
...
void func3(struct context3 *ctx3)
{
  ctx3->toRet = 0;
	...
	ctx3->wpentry = gm->workpool[ctx3->pid][0];

	if (!ctx3->wpentry)
		{
		gm->wpstat[ctx3->pid][0] = WPS_EMPTY;
    ctx3->toRet = 1;
    ctx3->retVal = (WPS_EMPTY);
    return;
		}

	gm->workpool[ctx3->pid][0] = ctx3->wpentry->next;
}
